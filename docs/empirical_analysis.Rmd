...
# TV & Women
Todo: Take resid_women_sales & resid_tv_sales and calculate the cross correlation
```{r}
women <- c("Brigitte", "freundin", "FÜR SIE")
tv <- c("TV Movie", "TV Spielfilm", "TV Today")


names(resid_tv_sales) <- tv
names(resid_tv_ads) <- tv
names(resid_women_sales) <- women
names(resid_women_ads) <- women
```


```{r tv_women_sales: Calculate CCF}
#FIXME: check if correlation is calculated correctly
